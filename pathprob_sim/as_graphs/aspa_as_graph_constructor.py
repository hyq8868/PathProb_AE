...
from .aspa_as_graph import ExtendedCAIDAASGraph
...
class ExtendedCAIDAASGraphConstructor(CAIDAASGraphConstructor):
    def __init__(
        self,
        ASGraphCollectorCls=CustomCAIDAASGraphCollector,
        ASGraphCls=ExtendedCAIDAASGraph,
        **kwargs,
    ) -> None:
        super().__init__(
            ASGraphCollectorCls=ASGraphCollectorCls,
            ASGraphCls=ASGraphCls,
            **kwargs,
        )

    def run(self, **kwargs):
        as_graph = super().run(**kwargs)

        if not isinstance(as_graph, ExtendedCAIDAASGraph):
            raise TypeError(f"Expected ExtendedCAIDAASGraph, got {type(as_graph)}")
